...
//Function Prototypes
int *fillAry(int,int);      //Fill the Array returns a pointer to an int
...
int *fillAry(int n, int modNum){
    ...
    for(int i=0;i<n;i++){
        *(array+i)=i%modNum; // fills array in incrementing order for digits 0-9
        //*(array+i)=rand()%modNum;
    }
...
int *mode(const int *array,int arySize){
    ...
    int tempary[arySize];
    int maxNum=0; //max Frequency
    int numModes=0; //counts maxnum
    ...
    if(numModes == 10) // resets to 0 if 10
    {
        numModes = 0;
    }
